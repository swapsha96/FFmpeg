 ...
 */
static ChannelInformation ff_mlp_ch_info[21] = {
...
{
    MLPEncodeContext *s = avctx->priv_data;
    ...
    case 44100 << 0:
        ...
        s->coded_sample_rate[0] = 0x08 + 0;
        s->fs                   = 0x08 + 1;
        break;
    case 44100 << 1:
        avctx->frame_size       = 40  << 1;
        s->coded_sample_rate[0] = 0x08 + 1;
        s->fs                   = 0x0C + 1;
        break;
    case 44100 << 2:
        s->substream_info      |= SUBSTREAM_INFO_HIGH_RATE;
        avctx->frame_size       = 40  << 2;
        s->coded_sample_rate[0] = 0x08 + 2;
        s->fs                   = 0x10 + 1;
        break;
    case 48000 << 0:
        avctx->frame_size       = 40  << 0;
        s->coded_sample_rate[0] = 0x00 + 0;
        s->fs                   = 0x08 + 2;
        break;
    case 48000 << 1:
        avctx->frame_size         = 40  << 1;
        s->coded_sample_rate[0] = 0x00 + 1;
        s->fs                   = 0x0C + 2;
        break;
    case 48000 << 2:
        s->substream_info      |= SUBSTREAM_INFO_HIGH_RATE;
        avctx->frame_size         = 40  << 2;
        s->coded_sample_rate[0] = 0x00 + 2;
        s->fs                   = 0x10 + 2;
        break;
    default:
    ...
    s->coded_sample_rate[1] = -1 & 0xf;

    /* TODO Keep count of bitrate and calculate real value. */
    s->coded_peak_bitrate = mlp_peak_bitrate(9600000, avctx->sample_rate);
    ...
    s->substream_info |= SUBSTREAM_INFO_ALWAYS_SET;
    if (avctx->channels <= 2) {
        s->substream_info |= SUBSTREAM_INFO_MAX_2_CHAN;
    }

    switch (avctx->sample_fmt) {
    case AV_SAMPLE_FMT_S16:
        s->coded_sample_fmt[0] = BITS_16;
        s->wordlength = 16;
        break;
    /* TODO 20 bits: */
    case AV_SAMPLE_FMT_S32:
        s->coded_sample_fmt[0] = BITS_24;
        s->wordlength = 24;
        break;
    default:
    ...
    s->coded_sample_fmt[1] = -1 & 0xf;

    avctx->coded_frame = avcodec_alloc_frame();

    s->dts = -avctx->frame_size;

    s->num_channels = avctx->channels + 2; /* +2 noise channels */
    s->one_sample_buffer_size = avctx->frame_size
                                * s->num_channels;
    /* TODO Let user pass major header interval as parameter. */
    s->max_restart_interval = MAJOR_HEADER_INTERVAL;

    if (avctx->compression_level == -1) {
        s->max_codebook_search = 3;
        s->min_restart_interval = MAJOR_HEADER_INTERVAL;
    } else {
        /* TODO Decide how much to test with the compression_level the user wants. */
        s->max_codebook_search = 3 * avctx->compression_level;
        s->min_restart_interval = MAJOR_HEADER_INTERVAL >> avctx->compression_level;
        if (!s->min_restart_interval)
            s->min_restart_interval = 1;
    }
    s->restart_intervals = s->max_restart_interval / s->min_restart_interval;

    /* TODO Let user pass parameters for LPC filter. */

    size = sizeof(int32_t) * avctx->frame_size * s->max_restart_interval;

    ret = ff_lpc_init(&s->lpc_ctx, size, 0, FF_LPC_TYPE_LEVINSON);

    size = sizeof(int32_t)
         * s->one_sample_buffer_size * s->max_restart_interval;

    s->major_scratch_buffer = av_malloc(size);
    if (!s->major_scratch_buffer) {
        av_log(avctx, AV_LOG_ERROR,
               "Not enough memory for buffering samples.\n");
        return -1;
    }

    s->major_inout_buffer = av_malloc(size);
    if (!s->major_inout_buffer) {
        av_log(avctx, AV_LOG_ERROR,
               "Not enough memory for buffering samples.\n");
        return -1;
    }
    ...
    s->channel_arrangement = avctx->channels - 1;
    s->num_substreams = 1;
    s->flags = FLAGS_DVDA;
    s->channel_occupancy = ff_mlp_ch_info[avctx->channels - 1].channel_occupancy;
    s->summary_info      = ff_mlp_ch_info[avctx->channels - 1].summary_info     ;

    size = sizeof(unsigned int) * s->max_restart_interval;

    s->frame_size = av_malloc(size);
    if (!s->frame_size)
        return -1;

    s->max_output_bits = av_malloc(size);
    if (!s->max_output_bits)
        return -1;

    size = sizeof(int32_t)
         * s->num_substreams * s->max_restart_interval;

    s->lossless_check_data = av_malloc(size);
    if (!s->lossless_check_data)
        return -1;

    for (index = 0; index < s->restart_intervals; index++) {
        s->seq_offset[index] = sum;
        s->seq_size  [index] = ((index + 1) * s->min_restart_interval) + 1;
        sum += s->seq_size[index];
    }
    s->sequence_size = sum;
    size = sizeof(ChannelParams)
         * s->restart_intervals * s->sequence_size * s->avctx->channels;
    s->channel_params = av_malloc(size);
    if (!s->channel_params) {
        av_log(avctx, AV_LOG_ERROR,
    ...
    size = sizeof(DecodingParams)
         * s->restart_intervals * s->sequence_size * s->num_substreams;
    s->decoding_params = av_malloc(size);
    if (!s->decoding_params) {
        av_log(avctx, AV_LOG_ERROR,
               "Not enough memory for analysis context.\n");
        return -1;
    }

    for (substr = 0; substr < s->num_substreams; substr++) {
        RestartHeader  *rh = &s->restart_header [substr];
    ...
    clear_channel_params(s, restart_channel_params);
    clear_decoding_params(s, restart_decoding_params);
                            ...
                            const AVFrame *frame, int *got_packet_ptr)
{
    ...
    if ((ret = ff_alloc_packet2(avctx, avpkt, frame->nb_samples * 10)) < 0)
        return ret;
...
static av_cold int mlp_encode_close(AVCodecContext *avctx)
{
    ...
    av_freep(&s->lossless_check_data);
    av_freep(&s->major_scratch_buffer);
    av_freep(&s->major_inout_buffer);
    ff_lpc_end(&s->lpc_ctx);
    av_freep(&s->decoding_params);
    av_freep(&s->channel_params);
    av_freep(&avctx->coded_frame);
    av_freep(&s->frame_size);
