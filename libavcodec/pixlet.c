...
static int init_decoder(AVCodecContext *avctx)
{
    ...
    if (!ctx->filter[0] || !ctx->filter[1] || !ctx->levels)
...
static int read_low_coeffs(AVCodecContext *avctx, uint16_t *dst, int size, int inw, int inh)
{
    PixletContext *ctx = avctx->priv_data;
...
static int read_highpass(AVCodecContext *avctx, uint8_t *ptr, int plane, AVFrame *frame)
{
    ...
    for (i = 0; i < ctx->levels * 3; i++) {
        ...
        if (magic != 0xDEADBEEF) {
            av_log(avctx, AV_LOG_ERROR, "wrong magic number: 0x%04x for plane %d, band %d\n", magic, plane, i);
            return AVERROR_INVALIDDATA;
        }
