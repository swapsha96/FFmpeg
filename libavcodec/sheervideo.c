...
static int get_vlc3(GetBitContext *gb, const uint8_t *len, const uint8_t *sym, const uint16_t *esc)
{
    int i, l, s;
    ...
    if (l > 8) {
        unsigned x;
        x = esc[(show_bits(gb, 16) & 0xFF) + (l << 8)];
        s += x & 0xff;
        l = x >> 8;
    }
    skip_bits_long(gb, l);
...
static int get_vlc4(GetBitContext *gb, const uint8_t *len, const uint16_t *sym, const uint16_t *esc)
{
    int i, l, s;
...
//             code = get_vlc4(&gb, l_u_yry10, s_u_yry10, lut_yry10) & 0x3FF;
