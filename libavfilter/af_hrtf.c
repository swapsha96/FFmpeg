...
/**
 * @file
 * Experimental audio filter that mixes 5.1 and 5.0 with matrix encoded
 * rear channels into headphone signal using FIR filtering with HRTF.
 */
...
typedef struct HRTFContext {
    ...
    float lf[DELAYBUFLEN], rf[DELAYBUFLEN],
          lr[DELAYBUFLEN], rr[DELAYBUFLEN],
          cf[DELAYBUFLEN], cr[DELAYBUFLEN];

    const float *cf_ir, *cr_ir,
                *af_ir, *ar_ir,
                *of_ir, *or_ir;
    ...
    float ba_l[DELAYBUFLEN], ba_r[DELAYBUFLEN];
    float ba_ir[BASSFILTLEN];

    int matrix_mode;  // Whether to matrix decode the rear center channel
    int decode_mode;  // How to decode the input

    float l_fwr, r_fwr,      // Full wave rectified (FWR) amplitudes and gain used
          lpr_fwr, lmr_fwr;  // to steer the active matrix decoding of fron channels
                             // (variable names lpr/lmr means Lt + Rt, Lt - Rt)

    float adapt_l_gain, adapt_r_gain,
          adapt_lpr_gain, adapt_lmr_gain;

    float fwrbuf_l[DELAYBUFLEN],   // Matrix input decoding require special FWR
          fwrbuf_r[DELAYBUFLEN],   // buffer, since the decoding is done in place
          fwrbuf_lr[DELAYBUFLEN],
          fwrbuf_rr[DELAYBUFLEN];

    float *rear_dlbuf;          // Rear channel delay buffer for matrix decoding

    float lr_fwr, rr_fwr,       // Full wave rectified amplitude and gain used to
          lrprr_fwr, lrmrr_fwr; // steer the active matrix decoding of center
                                // rear channel

    float adapt_lr_gain,    adapt_rr_gain,
          adapt_lrprr_gain, adapt_lrmrr_gain;

    int cyc_pos;             /* Cyclic position on the ring buffer */
} HRTFContext;
...
static const AVOption hrtf_options[] = {
    { "mode", "set the matrix mode", OFFSET(matrix_mode),  AV_OPT_TYPE_INT, {.i64=2}, 0, 2, A, "mode" },
...
static av_cold int init(AVFilterContext *ctx)
{
    ...
    fc = 2.0 * BASSFILTFREQ / 48000.0;
//    if (filter_design_fir(s->basslen, s->ba_ir, &fc, LP | KAISER, 4 * M_PI) ==-1) {
//      av_log(ctx, AV_LOG_ERROR, "Unable to design low-pass filter.\n");
...
static int query_formats(AVFilterContext *ctx)
{
    ...
    ff_add_channel_layout(&layouts, AV_CH_LAYOUT_5POINT0);
    ff_add_channel_layout(&layouts, AV_CH_LAYOUT_5POINT1);
 ...
 * sources.
 */
static void matrix_decode(int16_t *in, const int k, const int il,
...
static void update_channel(HRTFContext *s, int16_t *in, const int k)
{
    ...
    if (s->matrix_mode) {
       ...
       s->lrprr_fwr += abs(in[2] + in[3]) - fabs(s->fwrbuf_lr[fwr_pos] +
                                                 s->fwrbuf_rr[fwr_pos]);
       s->lrmrr_fwr += abs(in[2] - in[3]) - fabs(s->fwrbuf_lr[fwr_pos] -
                                                 s->fwrbuf_rr[fwr_pos]);
    }
    ...
    case HRTF_MIX_MATRIX2CH:
       ...
       s->fwrbuf_l[k] = in[0];
       s->fwrbuf_r[k] = in[1];
       matrix_decode(in, k, 0, 1, 1, s->dlbuflen, s->l_fwr, s->r_fwr,
                     s->lpr_fwr, s->lmr_fwr, &(s->adapt_l_gain),
                     &(s->adapt_r_gain), &(s->adapt_lpr_gain),
                     &(s->adapt_lmr_gain), s->lf, s->rf, s->lr, s->rr, s->cf);
       break;
...
static inline float filter_fir(register unsigned int n, const float* w,
                               const float* x)
{
    register float y; // Output
                  ...
                  const int offset)
{
    ...
    if (nk + k <= nx)
        return filter_fir(nk, sx + k, sk);
    else
        return filter_fir(nk + k - nx, sx, sk + nx - k) +
               filter_fir(nx - k, sx + k, sk);
}
...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    HRTFContext *s = ctx->priv;
    AVFrame *out;
    int16_t *src = (int16_t *)in->data[0];
    int16_t *dst;
    int16_t *end = src + in->nb_samples * inlink->channels;
    ...
    dst = (int16_t *)out->data[0];

    while (src < end) {
        const int k = s->cyc_pos;

        update_channel(s, src, k);
        ...
        /*
         * Bass compensation for the lower frequency cut of the HRTF.
         * A cross talk of the left and right channel is introduced to
        ...
        case HRTF_MIX_STEREO:
           ...
           dst[0] = av_clip_int16(left  + diff);
           dst[1] = av_clip_int16(right - diff);
           break;
        case HRTF_MIX_MATRIX2CH:
           /* Do attempt any stereo expansion with matrix encoded
              ...
              very unnatural. */
           dst[0] = av_clip_int16(left);
           dst[1] = av_clip_int16(right);
           break;
        }

        /* Next sample... */
        src += inlink->channels;
        dst += outlink->channels;
...
static const AVFilterPad hrtf_inputs[] = {
    {
        .name         = "default",
...
static const AVFilterPad hrtf_outputs[] = {
    {
        .name = "default",
        .type = AVMEDIA_TYPE_AUDIO,
    },
...
AVFilter ff_af_hrtf = {
    .name          = "hrtf",
    .description   = NULL_IF_CONFIG_SMALL("Head-related transfer function (HRTF)."),
    .init          = init,
    .query_formats = query_formats,
    .priv_size     = sizeof(HRTFContext),
    .priv_class    = &hrtf_class,
    .inputs        = hrtf_inputs,
    .outputs       = hrtf_outputs,
};
