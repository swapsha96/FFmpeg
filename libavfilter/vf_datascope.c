    ...
    for (y = 0; y < H && (y + s->y < inlink->h); y++) {
        for (x = slice_start; x < slice_end && (x + s->x < inlink->w); x++) {
            ...
            ff_fill_rectangle(&s->draw, &color, out->data, out->linesize,
                              xoff + x * C * 10, yoff + y * P * (8 + 4), C * 10, P * (8 + 4));

            for (p = 0; p < P; p++) {
                char text[256];

                snprintf(text, sizeof(text), format[C>>2], value[p]);
                draw_text(s, out, &reverse, xoff + x * C * 10 + 2, yoff + y * P * (8 + 4) + p * 10 + 2, text, 0);
            }
        }
    }
...
static int filter_frame(AVFilterLink *inlink, AVFrame *in)
{
    ...
    if (s->axis) {
        const int P = FFMAX(s->nb_planes, s->nb_comps);
        const int C = s->chars;
        const int Y = outlink->h / (P * 12);
        const int X = outlink->w / (C * 10);
        char text[256] = { 0 };
        int x, y;
        ...
        for (y = 0; y < Y; y++) {
            snprintf(text, sizeof(text), "%d", s->y + y);
            ymaxlen = FFMAX(ymaxlen, strlen(text));
            draw_text(s, out, &s->yellow, 2, xmaxlen + y * P * 12 + 4 * P, text, 0);
        }

        for (x = 0; x < X; x++) {
            snprintf(text, sizeof(text), "%d", s->x + x);
            draw_text(s, out, &s->yellow, ymaxlen + x * C * 10 + 2 * C, 0, text, 1);
        }
    }

    td.in = in; td.out = out, td.yoff = xmaxlen, td.xoff = ymaxlen;
    ctx->internal->execute(ctx, s->filter, &td, NULL, FFMIN(ctx->graph->nb_threads, outlink->w / 20));
