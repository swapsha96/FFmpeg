...
static void transform_step(float *input, float *output, const int size, const int lowSize, VagueContext *s)
{
    int i;

    symmetric_extension(input, size, 1, 1);

    for (i = NPAD; i < NPAD + lowSize; i++) {
...
static void invert_step(const float *input, float *output, float *temp, const int size, VagueContext *s)
{
    ...
    symmetric_extension(temp, lowSize, left_ext, right_ext);
