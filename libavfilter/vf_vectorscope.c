        ...
        if (s->is_yuv) {
            for (i = 0; i < in->height; i++) {
                ...
                for (j = 0; j < in->width; j++) {
                    const int x = src[px][iwx + j];
                    const int y = src[py][iwy + j];
                    const int pos = y * dlinesize + x;

                    dst[pd][pos] = FFMIN(dst[pd][pos] + 1, 255);
                    if (dst[3])
                        dst[3][pos] = 255;
                }
            }
        ...
        if (s->mode == COLOR) {
            for (i = 0; i < out->height; i++) {
                for (j = 0; j < out->width; j++) {
                    if (!dst[pd][i * out->linesize[pd] + j]) {
                        dst[px][i * out->linesize[px] + j] = j;
                        dst[py][i * out->linesize[py] + j] = i;
                    }
                }
            }
        }
    ...
    case COLOR2:
        if (s->is_yuv) {
            for (i = 0; i < in->height; i++) {
                ...
                for (j = 0; j < in->width; j++) {
                    const int x = src[px][iw1 + j];
                    const int y = src[py][iw2 + j];
                    const int pos = y * dlinesize + x;
                    if (!dst[pd][pos])
                        dst[pd][pos] = FFABS(128 - x) + FFABS(128 - y);
                    dst[px][pos] = x;
                    dst[py][pos] = y;
                    if (dst[3])
                        dst[3][pos] = 255;
    ...
    case COLOR3:
        for (i = 0; i < in->height; i++) {
            ...
            for (j = 0; j < in->width; j++) {
                ...
                dst[pd][pos] = FFMIN(255, dst[pd][pos] + 1);
                dst[px][pos] = x;
                dst[py][pos] = y;
                if (dst[3])
                    dst[3][pos] = 255;
