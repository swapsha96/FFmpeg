...
static void envelope_instant(WaveformContext *s, AVFrame *out, int component, int plane)
{
    ...
    const int start = s->estart[component];
    const int end = s->eend[component];
    uint8_t *dst;
    int x, y;
...
static void envelope_peak(WaveformContext *s, AVFrame *out, int component, int plane)
{
    const int dst_linesize = out->linesize[plane];
    const uint8_t bg = s->bg_color[plane];
    const int is_chroma = (plane == 1 || plane == 2);
    ...
    if (!s->mode) {
        ...
        if (s->envelope == 3)
            envelope_instant(s, out, component, plane);
...
static void envelope(WaveformContext *s, AVFrame *out, int component, int plane)
{
    if (s->envelope == 0) {
        return;
        ...
        envelope_peak(s, out, component, plane);
    }
}
...
static int config_output(AVFilterLink *outlink)
{
    ...
    for (p = 0; p < 4; p++) {
        ...
        for (i = 0; i < 4; i++) {
            s->emax[i][plane] = av_malloc(size);
            s->emin[i][plane] = av_malloc(size);
        }

        if (!s->emin[i][plane] || !s->emax[i][plane])
            return AVERROR(ENOMEM);
        ...
        for (i = 0; i < size / sizeof(int); i++) {
            for (k = 0; k < 4; k++) {
                s->emax[k][plane][i] = s->estart[plane];
                s->emin[k][plane][i] = s->eend[plane];
            }
        }
    }
