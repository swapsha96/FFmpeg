...
static const AVOption waveform_options[] = {
    ...
    { "select", "select row/column", OFFSET(select), AV_OPT_TYPE_INT, {.i64=-1}, -1, INT_MAX, FLAGS },
    { "s",      "select row/column", OFFSET(select), AV_OPT_TYPE_INT, {.i64=-1}, -1, INT_MAX, FLAGS },
    { NULL }
};
...
static int config_input(AVFilterLink *inlink)
{
    ...
    if (s->mode) {
        if (s->select >= inlink->h) {
            av_log(ctx, AV_LOG_ERROR, "Selected row is out of range.\n");
            return AVERROR(EINVAL);
        }
    } else {
        if (s->select >= inlink->w) {
            av_log(ctx, AV_LOG_ERROR, "Selected column is out of range.\n");
            return AVERROR(EINVAL);
        }
    }
                         ...
                         int component, int intensity, int offset, int col_mode)
{
    ...
    const int start_y = !s->mode || s->select == -1 ? 0 : s->select >> shift_h;
    const int end_y = !s->mode || s->select == -1 ? src_h : start_y + 1;
    const int start_x = s->mode || s->select == -1 ? 0 : s->select >> shift_w;
    const int end_x = s->mode || s->select == -1 ? src_w : start_x + 1;
    const uint8_t *src_data = in->data[plane] + start_y * src_linesize;
    ...
    for (y = start_y; y < end_y; y++) {
        const uint8_t *src_data_end = src_data + end_x;
        dst = dst_line;
        for (p = src_data + start_x; p < src_data_end; p++) {
            uint8_t *target;
            if (col_mode) {
